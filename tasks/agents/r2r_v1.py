...
class R2RAgent(MP3DAgent):
    ...
    def get_navigation_graph_prompt(instruction: str,
                                    hist_num: list,
                                    cand_num: int,
                                    cls_token: str,
                                    graph_memory_snippets: list[str]) -> str:

        gm_text = '\n'.join([f'- {m}' for m in graph_memory_snippets]) if graph_memory_snippets else '- (empty)'
        obs_text = ' '.join([f'({i}) <cand>' if i > 0 else '(0) stop' for i in range(cand_num)])
        ...
        prompt = (
            ...
            f'### Candidate\n{obs_text}\n'
            '### Task\n'
            '1) Make/keep a subgoal plan (<=4). 2) Pick current subgoal. 3) Choose ONE action.\n'
            ...
            '"hypotheses":[{"entity":"...","polarity":"pos|neg","strength":0.0}],'
            '"evidence":[{"entity":"...","type":"seen|not_seen","confidence":0.0,"source":"claimed"}],'
            '"rationale":"<=2 sentences"'
            '}\n'
            f'Action token: {cls_token}\n'
            'Rules: action in candidates; numbers are numbers; do not hallucinate; evidence source="claimed"; notes <=8 words.\n'
        )
        return prompt
